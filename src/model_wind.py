...
class Residual_9_9(nn.Module):  #@save
    '''
    残差连接模块
    '''    
    def __init__(self, input_channels, num_channels, use_1x1conv=False,use_sp=False, strides=(1,1,1)):
        ...
        #self.ch_at=ChannelAttention(input_channels,input_channels)
        #self.sp_at=SpatialAttention()
        self.conv1 = nn.Conv3d(input_channels,num_channels, kernel_size=(1,9,9), padding=(0,4,4), stride=strides)
        
        self.conv1_bn=nn.BatchNorm3d(num_channels)
        self.conv1_relu=nn.ReLU()
        
        self.conv2 = nn.Conv3d(num_channels, num_channels, kernel_size=(1,9,9), padding=(0,4,4))
        self.conv2_relu=nn.ReLU()
        self.conv2_bn=nn.BatchNorm3d(num_channels)
        
        if use_sp:
            self.ch_at0=ChannelAttention(num_channels,num_channels)
            self.sp_at0=SpatialAttention()
        else:
            self.ch_at0=None
            self.sp_at0=None
        # self.bn1=nn.BatchNorm3d(num_channels)
...
class Residual(nn.Module):
    ...
    残差连接模块
    '''    
    def __init__(self, input_channels, num_channels, use_1x1conv=False, use_sp=False, use_ch=False,use_pool=False, strides=(1,1,1),sa_k=3,sa_s=1,sa_p=1,activate_f='relu'):
        super().__init__()
...
class Residual_5_5(nn.Module):  #@save
    '''
    残差连接模块
    '''    
    def __init__(self, input_channels, num_channels, use_1x1conv=False, strides=(1,1,1)):
        super().__init__()
        
        #self.ch_at=ChannelAttention(input_channels,input_channels)
        #self.sp_at=SpatialAttention()
        self.conv1 = nn.Conv3d(input_channels,num_channels, kernel_size=(5,5,5), padding=(2,2,2), stride=strides)
        
        self.conv1_bn=nn.BatchNorm3d(num_channels)
        self.conv1_relu=nn.ReLU()
        #nn.ReLU()
        
        self.conv2 = nn.Conv3d(num_channels, num_channels, kernel_size=(5,5,5), padding=(2,2,2))
        self.conv2_relu=nn.ReLU()
        self.conv2_bn=nn.BatchNorm3d(num_channels)
...
class WeatherCNN3D(nn.Module):
    def __init__(self,mattention=0.3,mlp_drop=0.3,fc=256,num_heads=16,at_fun='relu'):
        ...
        # #init 卷积层
        # self.resnet1=Residual_7_7(14,32,False,False,strides=(1,2,2))  #10*10 
        ...
        # # 3*3*3 卷积层
        # self.resnet5_3=Residual(256,512,True,use_sp=False,use_ch=False,use_pool=False,strides=(1,2,2))
