...
import pandas as pd
import xarray as xr
from config.config import wrf_extent,sd_wind_outprefix,sd_wind_prefix
...
def read_file(ncfile:str):
    ...
    T2M=getvar(ds,'T2',timeidx=ALL_TIMES).data[0,:,:]
    
    
    WS_100=getvar(ds,'WSPD10MAX',timeidx=ALL_TIMES).data[0,:,:]
    ...
    item_dict['T2M']=T2M_line
    item_dict['WS100']=WS100_line
...
def grid_data(lon_line,lat_line,data_var,time):
    ...
    T2M=griddata((lon_line,lat_line),data_var['T2M'],(gridlon,gridlat),method='nearest',fill_value=-9999) #12
    Ws100=griddata((lon_line,lat_line),data_var['WS100'],(gridlon,gridlat),method='nearest',fill_value=-9999) #12
    
    U10_da=xr.DataArray(U10, coords={'latitude': lat, 'longitude': lon}, dims=('latitude', 'longitude')) 
    
    V10_da=xr.DataArray(V10, coords={'latitude': lat, 'longitude': lon}, dims=('latitude', 'longitude')) 
    
    T2M_da=xr.DataArray(T2M, coords={'latitude': lat, 'longitude': lon}, dims=('latitude', 'longitude')) 
    
    Ws100=xr.DataArray(Ws100, coords={'latitude': lat, 'longitude': lon}, dims=('latitude', 'longitude')) 
...
def extract_one_day(forecast_time:datetime.datetime):
    ...
    print(time_str)
    input_files=glob(sd_wind_prefix+'/'+time_str+'/hhups_sr_d01*')
    print(sd_wind_prefix+'/'+time_str)
    ...
    if len(input_files)>0:
        ...
        #print(files)
        out_prefix=sd_wind_outprefix
        # if not os.path.exists(out_prefix):
        #     os.mkdir(out_prefix)
        
        
        #out_time_str=forecast_time.strftime('%Y%m%d%H')
        outfilename='sd_wrf_wind_'+time_str+'.nc'
